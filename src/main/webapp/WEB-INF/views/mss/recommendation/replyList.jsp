	 	 ...
	 	 <c:choose>
	 	 	<c:when test="${empty row.userID}" >
				<p class="comment-board" style="float: left;">
					${row.rnickname}&nbsp;<span data-toggle="modal" data-target="#boardModal2"
						bcid="76748" class="deleteComment comment-tip" type="board"  onclick="deleteComment('${row.rno}')">삭제</span><span
						class="board-ip"> <c:out value="${fn:substring(row.rip,0,8)}"  escapeXml="false" />***
						 </span>
				</p>
				<pre class="comment-board" style="color: #444; padding-right: 35px;">${row.content}</pre>	 	 			 	 	
	 	 	</c:when>
			<c:when test="${row.userID eq sessionScope.USER.userID}">
				<p class="comment-board logined" style="float:left;">${row.rnickname }&nbsp;
	<span uid="3959" bcid="76801" class="deleteLoginedComment comment-tip" type="board"  onclick="replyDelete('${row.rno}')">삭제</span>
				<span class="board-ip moment"><fmt:formatDate value="${row.regdate}"  pattern="yyyy-MM-dd hh:mm:ss"/></span></p>
					<pre class="comment-board" style="color: #444; padding-right:35px;">${row.content}</pre>
			</c:when>
			<c:otherwise>
			<p class="comment-board logined" style="float:left;">${row.rnickname}&nbsp;
